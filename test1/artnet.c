...
static unsigned char
    ...
    ip_address[4] = {10, 1, 5, 57},
    mac_address[6] = {0x2c, 0x4d, 0x54, 0x56, 0xbc, 0x63},
    NetSwitch = (PortAddress >> 8) & 0x7F,
    SubSwitch = (PortAddress >> 4) & 0x0F;

static const unsigned char
    PortName[18] = "STM32_ARTNET",
    LongName[64] = "STM32 Art-Net"
    ;

static unsigned char
    NodeReport[64] = "Hello World!"
    ;
...
int artnet_compose_OpPollReply(unsigned char* buf)
{
    ...
    PUT_BYTE(buf_tail, EstaManLo);
    PUT_BYTE(buf_tail, EstaManHi);
    ...
    PUT_BYTE(buf_tail, mac_address[0]);
    PUT_BYTE(buf_tail, mac_address[1]);
    PUT_BYTE(buf_tail, mac_address[2]);
    PUT_BYTE(buf_tail, mac_address[3]);
    PUT_BYTE(buf_tail, mac_address[4]);
    PUT_BYTE(buf_tail, mac_address[5]);
